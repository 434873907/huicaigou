...
import com.chengzi.apiunion.procurement.admin.web.formbean.template.UpdateTemplateItemForm;
 ...
 * 更新模板商品
 */
public class UpdateTemplateItemController extends AbstractApiController<UpdateTemplateItemForm> {

    private static final int MAX_IMG_NUM = 8;

    @Override
    protected void onBindAndValidate(HttpServletRequest request, UpdateTemplateItemForm command, BindException errors) throws Exception {
    ...
    @Override
    protected Result<?> doBiz(HttpServletRequest request, HttpServletResponse response, UpdateTemplateItemForm command) throws Exception {
       ...
       if(templateItemId == null || templateItemId <=0) {
           return Result.buildOpFailedResult("模板ID参数错误");
       }
        ...
        try {
            ...
            Result<String> result = templateItemService.updateTemplateItem(templateItemDO, templateItemDetailDO, command.getSkuNameValues(),
    ...
    private SupplierTemplateItemDO buildTemplateItemDO(UpdateTemplateItemForm itemForm) {
        SupplierTemplateItemDO templateItemDO = new SupplierTemplateItemDO();

        templateItemDO.setId(itemForm.getTemplateItemId());
