...
var ajax = (option) => { //ajax请求方法
    ...
    nextMethod(_startIntercept, setting, () => {
        ...
        if ((location.hostname == 'localhost' || ~location.host.indexOf('9090')) && !~setting.url.indexOf('http') && location.port != '9050') {
        ...
        xhr.onreadystatechange = () => {
            ...
            if(xhr.readyState == 4) {    //请求完成，api调用成功
                if(xhr.status == 200) {
                    ...
                    if (key == getPageKey()) nextMethod(_endIntercept,response,setting.success)
                } else {
